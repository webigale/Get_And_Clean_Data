...
## Column 1 is the Subject and column 2 is the Activity.
ms_df <- data_set[, sort(c(1, 2, mean_fields, std_fields))]

## Uses descriptive activity names to name the activities in the data set. (STEP 3)
act_lbl <- read.table("activity_labels.txt")
ms_df$activity <- as.factor(act_lbl$V2[ms_df$activity])
